...
describe("06 - Checkbox 2", () => {
  ...
  it("puede marcarse y desmarcarse alternando su estado", async () => {
    const cb = await controlador.findElement(By.css("input[name='checkboxes[]'][value='cb2']"));
    ...
    await capturar(controlador, "06-checkbox2-estado-inicial");

    await cb.click();
    await esperar(TIEMPOS.ACCION_PAUSA);
    await capturar(controlador, "06-checkbox2-primer-click");
    expect(await cb.isSelected()).toBe(!estadoInicial);

    await cb.click();
    await esperar(TIEMPOS.ACCION_PAUSA);
    await capturar(controlador, "06-checkbox2-segundo-click");
    expect(await cb.isSelected()).toBe(estadoInicial);
  }, TIMEOUTS.TEST_LARGO);
